...
WHERE EXISTS (
    SELECT 1
    FROM genre g
    WHERE g.genre_id = t.genre_id AND g.name = 'Rock'
)
...
sum( invo.unit_price * invo.quantity) as total_sales 
from invoice_line invo 
join track tr on tr.track_id = invo.track_id
...
with recursive 
 sales_per_country as 
 	(
		...
		from invoice_line
		join invoice on invoice_line.invoice_id = invoice.invoice_id
		join customer on custome.customer_id = invoice.customer_id
		join track on track.track_id = invoice_line.track_id
		join genre on genre.genre_id = track.genre_id
		group by 2,3,4
		order by 1
...
with customer_with_country as(
	...
	row_number() over(partition by billing_country order by sum(total)) as row_no 
	from invoice
	join customer on customer.customer_id = invoice.customer_id
	group by 1,2,3,4
